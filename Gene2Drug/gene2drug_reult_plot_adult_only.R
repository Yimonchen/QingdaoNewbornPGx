...
#—————————————————— Pediatric&Adult ——————————————————#
# drug usage table
mydata_usage <- read.table("data_adult_usage_modified.tsv", header = T, sep = "\t")
...
mydata_gene <- read.table("data_adult_gene_modified.tsv", header = T, sep = "\t")
...
mydata_level <- read.table("data_adult_level_modified.tsv", header = T, sep = "\t")
...
mydata_category <- read.table("data_adult_category_modified.tsv", header = T, sep = "\t")
...
p11 <- ggplot() +
  ...
  scale_fill_manual(values = c("#1F78B4",  "#E31A1C", "#FDBF6F", "#B15928"),
                    name = "Drug Usage",
...
ggsave("p2.pdf", width = 3, height = 8, dpi = 300, units = c("in"))

p13 <- ggplot() +
  geom_bar(data=mydata_category,aes(x=drug,y=0.1,fill=ATC1stLabel), 
           position = position_stack(),stat="identity", show.legend=T) +
  scale_fill_manual(values = rep("white", times=11),
	            name = "Drug Category\n(ATC 1st Level)",
	            guide = guide_legend(nrow = 1, title.position = "top")) +
...
ggsave("p3.pdf", width = 4, height = 8, dpi = 300, units = c("in"))
